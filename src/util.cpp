...
const char * const BITCOIN_CONF_FILENAME = "wuazi.conf";
const char * const BITCOIN_PID_FILENAME = "wuazid.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "wuazi" is a composite category enabling all Wuazi-related debug output
            if(ptrCategory->count(string("wuazi"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WuaziCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WuaziCore
    // Mac: ~/Library/Application Support/WuaziCore
    // Unix: ~/.wuazicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WuaziCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WuaziCore";
#else
    // Unix
    return pathRet / ".wuazicore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty wuazi.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override wuazi.conf
