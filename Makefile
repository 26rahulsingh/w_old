...
pkgdatadir = $(datadir)/wuazicore
pkgincludedir = $(includedir)/wuazicore
pkglibdir = $(libdir)/wuazicore
pkglibexecdir = $(libexecdir)/wuazicore
...
CONFIG_HEADER = $(top_builddir)/src/config/wuazi-config.h
CONFIG_CLEAN_FILES = libwuaziconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libwuaziconsensus.pc.in \
	...
	$(top_srcdir)/src/config/wuazi-config.h.in \
...
ACLOCAL = ${SHELL} /home/devjs/git/wuazi/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/devjs/git/wuazi/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/devjs/git/wuazi/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/devjs/git/wuazi/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/devjs/git/wuazi/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = wuazicore
PACKAGE_BUGREPORT = https://github.com/wuazipay/wuazi/issues
PACKAGE_NAME = Wuazi Core
PACKAGE_STRING = Wuazi Core 0.12.2
PACKAGE_TARNAME = wuazicore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /home/devjs/git/wuazi
abs_srcdir = /home/devjs/git/wuazi
abs_top_builddir = /home/devjs/git/wuazi
abs_top_srcdir = /home/devjs/git/wuazi
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/devjs/git/wuazi/build-aux/install-sh
...
pkgconfig_DATA = libwuaziconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/wuazid$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/wuazi-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/wuazi-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Wuazi-Qt.app
OSX_DMG = Wuazi-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_wuazi_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_wuazi_coverage.info test_wuazi.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Wuazi-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/wuazi-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/wuazi-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/wuazi-config.h
$(top_srcdir)/src/config/wuazi-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/wuazi-config.h src/config/stamp-h1
libwuaziconsensus.pc: $(top_builddir)/config.status $(srcdir)/libwuaziconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Wuazi-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Wuazi-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Wuazi-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Wuazi-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_wuazi.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_wuazi -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_wuazi_filtered.info: test_wuazi.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_wuazi_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_wuazi_filtered.info
...
#test_wuazi_coverage.info: baseline_filtered_combined.info test_wuazi_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_wuazi_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_wuazi_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_wuazi_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_wuazi.coverage/.dirstamp:  test_wuazi_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_wuazi.coverage/.dirstamp total.coverage/.dirstamp

# Wuazi: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_wuazi.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
